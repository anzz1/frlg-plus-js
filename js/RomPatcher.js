...
try{
	...
	webWorkerCrc.onmessage = event => { // listen for events from the worker
		...
		var valid = validateSource();
		setTabApplyEnabled(true, valid);
	};
...
function parseCustomPatch(customPatch){
	...
	if(typeof patch.validateMD5 === 'undefined'){
		if(typeof customPatch.md5==='string'){
			patch.validateMD5=function(romFile,headerSize){
				return customPatch.md5===padZeroes(md5(romFile, headerSize), 16)
			}
		}else if(typeof customPatch.md5==='object'){
			patch.validateMD5=function(romFile,headerSize){
				for(var i=0; i<customPatch.md5.length; i++)
					if(customPatch.md5[i]===padZeroes(md5(romFile, headerSize), 16))
						return true;
				return false;
			}
		}
...
addEvent(window,'load',function(){
	...
	addEvent(el('squirrels'), 'mouseover', function(){
		el('squrel').style.visibility='visible';
		clearTimeout(myTimeout);
        myTimeout = window.setTimeout(function(){
			el('squrel').style.visibility='hidden';
		}, 3000);
	});
});
...
function validateSource(){
    var valid = false;
	if(patch && romFile && (typeof patch.validateCRC !== 'undefined' || typeof patch.validateMD5 !== 'undefined')){
		if(typeof patch.validateCRC !== 'undefined'){
			if(patch.validateCRC(romFile, false)){
		...
		if(typeof patch.validateMD5 !== 'undefined'){
			if(patch.validateMD5(romFile, false)){
				el('md5').className='valid';
				setMessage('apply');
				valid = true;
			}else{
				el('md5').className='invalid';
				setMessage('apply', 'error_crc_input', 'error');
			}
		}
	}else{
...
//	}
//}
function setTabApplyEnabled(status, valid){
	setElementEnabled('input-file-rom', status);
	setElementEnabled('input-file-patch', status);
	if(romFile && status && valid && (patch || isCustomPatcherEnabled())){
